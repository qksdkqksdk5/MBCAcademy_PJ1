...
def run_parking_poly_regression(df, degree=2, alpha=0.1):
    ...
    ax.plot(
        ...
        linestyle="--",
        linewidth=3,
        label=f"다항 회귀 (degree={degree})"
    )

    ax.set_title(
        "자동차 수와 주차면 수의 비선형 관계 (다항 회귀)",
        fontsize=18,
        fontweight="bold",
        pad=20
    )
    ax.set_xlabel("자동차 수 (단위: 만 대)", fontsize=14)
    ax.set_ylabel("주차면 수 (단위: 만 면)", fontsize=14)
